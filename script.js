...
/* =========================================================
   ...
   Click anywhere on a product card to open the product interface.
   The WhatsApp link still stays available inside the opened interface.
   Dragging the rail sideways will not open the modal.
   ========================================================= */
(function(){
  let downX = 0;
  let downY = 0;
  ...
  document.addEventListener("pointermove", function(event){
    ...
    if (Math.abs(event.clientX - downX) > 10 || Math.abs(event.clientY - downY) > 10) {
      didDragCard = true;
    }
  }, true);
  ...
  document.addEventListener("click", function(event){
    ...
    // Do not open while the user is dragging the rail sideways.
    if (didDragCard) return;
    if (typeof railDidDrag !== "undefined" && railDidDrag) return;
