...
/* =========================================================
   RAIL WIRE HANGER SAFETY INSERT
   Adds the visible long line from the fixed rail to each card.
   ========================================================= */
(function(){
  function ensureWireHangers(){
    const grid = document.querySelector("#product-grid");
    if (!grid) return;
    grid.querySelectorAll(".product-card:not(.product-card-skeleton)").forEach(card => {
      if (card.querySelector(".rail-wire-hanger")) return;
      const hanger = document.createElement("div");
      hanger.className = "rail-wire-hanger";
      hanger.setAttribute("aria-hidden", "true");
      hanger.innerHTML = '<span class="rail-loop"></span><span class="rail-wire"></span>';
      card.prepend(hanger);
    });
  }

  window.ensureWireHangers = ensureWireHangers;

  document.addEventListener("DOMContentLoaded", ensureWireHangers);
  window.addEventListener("load", ensureWireHangers);

  const grid = document.querySelector("#product-grid");
  if (grid) {
    new MutationObserver(() => requestAnimationFrame(ensureWireHangers))
      .observe(grid, { childList:true, subtree:false });
  }

  setTimeout(ensureWireHangers, 200);
  setTimeout(ensureWireHangers, 800);
  setTimeout(ensureWireHangers, 1600);
})();



/* =========================================================
   WHOLE PRODUCT CARD CLICK SAFETY
   Guarantees every click on a product card opens the interface.
   ========================================================= */
document.addEventListener("click", function(event){
  const card = event.target.closest("#product-grid .product-card");
  if (!card || card.classList.contains("product-card-skeleton")) return;
  if (typeof railDragJustHappened !== "undefined" && railDragJustHappened) return;

  event.preventDefault();
  event.stopPropagation();

  if (typeof openProductInterface === "function") {
    openProductInterface(card);
  }
}, true);
