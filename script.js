...
let railDragJustHappened = false;

const CACHE_KEY = "baadjie_products_cache_last_fix";
const CACHE_TIME_KEY = "baadjie_products_cache_time_last_fix";
...
function setupRail() {
  ...
  const stopDrag = () => {
    pointerDown = false;
    setTimeout(() => { railDragJustHappened = false; }, 180);
  };
...
document.addEventListener("click", event => {
  ...
  if (card && !card.classList.contains("product-card-skeleton")) {
    if (railDragJustHappened) return;
